                                 ...
                                 GError **error)
{
	...
	*out_changed = 0;
	...
	while (g_variant_iter_next (&iter, "{&sv}", &v_key, &v_val)) {
		...
		for (attr_data = team_attr_datas; attr_data < &team_attr_datas[G_N_ELEMENTS (team_attr_datas)]; attr_data++) {
			if (   _team_attr_data_is_relevant (attr_data, self->d.is_port)
			    && nm_streq (v_key, attr_data->dbus_name))
				break;
		}

		if (attr_data >= &team_attr_datas[G_N_ELEMENTS (team_attr_datas)]) {
	...
	*out_changed |= nm_team_setting_config_set (self,
	...
	if (variants[NM_TEAM_ATTRIBUTE_CONFIG]) {
		...
		for (attr_data = &team_attr_datas[NM_TEAM_ATTRIBUTE_CONFIG + 1]; attr_data < &team_attr_datas[G_N_ELEMENTS (team_attr_datas)]; attr_data++) {
			NMValueTypUnion val;
			gboolean changed = 0;
			...
			if (attr_data->value_type != NM_VALUE_TYPE_UNSPEC) {
				...
				changed = _nm_team_setting_value_set (self,
				                                      attr_data->team_attr,
				                                      attr_data->value_type,
				                                      &val);
			} else if (attr_data->team_attr == NM_TEAM_ATTRIBUTE_LINK_WATCHERS) {
				changed = nm_team_setting_value_link_watchers_set_list (self,
				...
				changed = nm_team_setting_value_master_runner_tx_hash_set_list (self, strv, NM_MIN (len, (gsize) G_MAXUINT));
			} else
				nm_assert_not_reached ();
